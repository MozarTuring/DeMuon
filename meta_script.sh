...
if [[ "$1" == "slurm" ]]; then
    ...
    export RUN_DIR="../runs/${3}/${JWM_COMMIT_ID}/"
    source remote.sh
...
cp -R . ${RUN_DIR}
cd ${RUN_DIR}
sbatch --time=1-00:00:00 --nodes=${JWM_NODES_NUM} --gpus-per-node=${JWM_GPU_TYPE}:${JWM_GPU_NUM} --job-name="${JWM_COMMIT_ID}" slurm.sh "${JWM_GPU_NUM}" && while ! [ -f slurm_out.log ]; do sleep 1; done && tail -f slurm_out.log
    ...
    git submodule foreach 'git add -A && (git commit -m "v" || true)' &&
        ...
        tar --disable-copyfile --exclude='.git' --exclude='.DS_Store' -czf tmp.tar.gz "$2" common_tools/meta_script.sh &&
        scp tmp.tar.gz "$1":~/project_remote_jwm/

    echo ${last_commit}

    osascript - "$2" "${last_commit}" "$3" <<'EOF'
on run argv
    ...
    tell application "iTerm2"
        tell current window
            tell current session
                ...
                delay 2
                write text "cd ~/project_remote_jwm && tar -xzf tmp.tar.gz && cd " & p1 & " && source ../common_tools/meta_script.sh " & p3 p2 p1
            end tell
        end tell
    end tell
end run
