...
def check_nan_inf(tensor_name, inp_tenosr, round):
    if torch.isnan(inp_tenosr).any() or torch.isinf(inp_tenosr).any():
        jwp(f"NaN or Inf detected in parameter {tensor_name} at round {round}")
        jwp(inp_tenosr)
        1/0
